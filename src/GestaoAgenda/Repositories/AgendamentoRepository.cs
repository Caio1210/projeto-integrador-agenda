...
public class AgendamentoRepository(GestaoAgendaContext context) : IAgendamentoRepository
{
    ...
    public Task<List<Agendamento>> GetAgendamentosAsync() =>
        _db.Agendamentos
            .Include(a => a.Cliente)
            .Include(a => a.Servico)
            .OrderBy(a => a.Data)
    ...
    public Task<Agendamento?> GetAgendamentoAsync(int? id) =>
        _db.Agendamentos
            .Include(a => a.Cliente)
            .Include(a => a.Servico)
            .FirstOrDefaultAsync(a => a.Id == id);

    public async Task<bool> HorarioDisponivelAsync(DateTime data, TimeSpan hora, int? id = null)
    {
    ...
    {
        var item = await _db.Agendamentos.FindAsync(id);

        if (item != null)
            _db.Agendamentos.Remove(item);
